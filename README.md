...
### フォームだけは自前の翻訳を使っています
翻訳プロキシは `<input>` や `<textarea>` にカーソルが入った時点で
「このフォームはサポートされていません」という警告を出し、入力を止めます。
（`<form>` の有無は関係ありません。実測で確認済み）
このままでは英語以外の言語で予約も問い合わせもできないので、役割を分けました。

| 何を | どこで | 言語 |
|---|---|---|
| 読む（イベント説明文を含む本文全部） | `*.translate.goog` | Google 翻訳 |
| 入力する（予約フォーム・問い合わせフォーム） | 本来のドメイン `?lang=xx` | `i18n.js` の自前翻訳 |

翻訳ページの予約欄には入力欄の代わりに母語の案内とボタンが出て、押すと
`https://ドメイン/event.html?id=…&lang=ja#book` へ移動します。移動先では
フォームと確認ページが日本語で表示され、警告も出ません。`?lang=` は
サイト内リンクに自動で引き継がれます。

**文言を足すとき**は `i18n.js` の `en` に書いてから、8言語すべてに同じキーを
足してください（訳が無いキーは英語にフォールバックします）。
HTML 側は `data-i18n="キー"` / `data-i18n-ph="キー"`（placeholder 用）、
JS 側は `t('キー')` で引けます。
