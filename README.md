...
### フォームを `<form>` にしていない理由
翻訳プロキシはページ内に `<form>` を見つけると「このフォームはサポートされていません」
というダイアログを出し、入力そのものを止めてしまいます。翻訳ページから予約や
問い合わせができなくなるため、**このサイトには `<form>` 要素がありません**。
送信はもともと全て JS（fetch）で行っていて `<form>` の submit 機能は使っていなかったので、
コンテナを `<div>` にし、`core.js` の `wireForm()` で「ボタン押下」と「Enter キー」を拾っています。

> フォームを追加するときも `<form>` は使わず、`<div>` ＋ `data-submit` 付きボタン ＋
> `wireForm('#そのdiv', handler)` の形にしてください。
